...
void MainWindow::setupCapturedImageTab()
{
    ...
    m_dateEdit->setStyleSheet("QDateEdit { background-color: white; color: black; padding: 8px; border: 1px solid #ccc; border-radius: 4px; }");
    ...
    m_hourSpinBox->setValue(QTime::currentTime().hour());
    m_hourSpinBox->setSuffix(QString("시 ~ %1시").arg(QTime::currentTime().hour() + 1));
    m_hourSpinBox->setStyleSheet("QSpinBox { background-color: white; color: black; padding: 8px; border: 1px solid #ccc; border-radius: 4px; }");
    connect(m_hourSpinBox, QOverload<int>::of(&QSpinBox::valueChanged), this, &MainWindow::onHourChanged);
    controlLayout->addWidget(m_hourSpinBox);
...
void MainWindow::onRequestImagesClicked()
{
    ...
    int selectedHour = m_hourSpinBox->value();
    ...
    m_tcpCommunicator->requestImagesByDateAndHour(selectedDate, selectedHour);
    
    qDebug() << QString("이미지 요청: %1, %2시~%3시").arg(selectedDate).arg(selectedHour).arg(selectedHour + 1);
}
