...
void MainWindow::setupNetworkConnection()
{
    ...
    // 기존 시그널 연결
    connect(m_tcpCommunicator, &TcpCommunicator::connectionStatusChanged, this, &MainWindow::onTcpConnected);
    connect(m_tcpCommunicator, &TcpCommunicator::connectionStatusChanged, this, &MainWindow::onTcpDisconnected);
...
{
    if (success) {
        QMessageBox::information(this, "좌표 전송 결과", "좌표 전송 성공: " + message);
    } else {
        QMessageBox::warning(this, "좌표 전송 결과", "좌표 전송 실패: " + message);
    }
}
